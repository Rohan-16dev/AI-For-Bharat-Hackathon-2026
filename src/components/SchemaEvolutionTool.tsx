...
import { analyzeDataAnomaly, AIError } from '../services/geminiService';
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="flex justify-between items-center">
          <div className="flex-1 mr-4">
            {error && (
              typeof error === 'string' ? (
                <span className="text-[10px] text-red-500 font-bold flex items-center gap-1"><AlertCircle className="w-3 h-3" /> {error}</span>
              ) : (
                <AIErrorAlert error={error} onRetry={handleAnalyze} className="py-2 px-3" />
              )
            )}
          </div>
          <button 
        ...
        <AnimatePresence>
          {analysis && (
            ...
            >
              ...
              <div className="bg-bg/50 p-4 rounded-lg border border-border text-xs text-text-main">
                <ReactMarkdown
                  components={{
